...
Direct API clients provide this opaque key. For Candidate creation through the web application, the browser supplies only a logical attempt nonce and the same-origin Next.js server derives the raw API key from authenticated session, actor, validated tenant, attempt, and canonical payload context. The browser neither chooses nor receives that raw key.
