...
The six current mutation routes require a validated `Idempotency-Key`. Records are scoped by the validated tenant and membership, a trusted operation identifier, and the client key; client body fields cannot select any part of this scope. A SHA-256 fingerprint covers the canonical, validated application input and any route ID that identifies the mutation. The stored result contains only the shaped public response DTO and original success status.
