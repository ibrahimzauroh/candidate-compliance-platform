...
The current business UI provides a server-paginated Candidate list, Candidate creation/detail, and basic compliance-document list/create/read views. Search, exact-email filtering, partial-role filtering, and Candidate/document page state map to the API rather than being recreated over unbounded client-side datasets. Candidate and document creation use browser attempt nonces, but the same-origin server derives each raw backend idempotency key from authenticated session, actor, validated tenant, target aggregate, attempt, and canonical payload context. The browser cannot choose or observe those keys. Document creation follows the metadata-only JSON contract and begins at `DRAFT`; approval, correction, version history, verification, CV, and audit workflows remain deferred.
